...
<div class="homepageDiv">
    ...
    <script>
        $(function () {
            ...
            $(document).on('mouseenter', 'div.eachCategory', function () {
                var cid = $(this).attr("cid");
                $("div.productsAsideCategorys[cid=" + cid + "]").show();
            });
            $(document).on('mouseleave', 'div.eachCategory', function () {
                var cid = $(this).attr("cid");
                $("div.productsAsideCategorys[cid=" + cid + "]").hide();
            });
            $(document).on('mouseenter', 'div.productsAsideCategorys', function () {
                $(this).show();
            });
            $(document).on('mouseleave', 'div.productsAsideCategorys', function () {
                $(this).hide();
            });
    ...
    <img src="${APP_PATH}/img/site/catear.png" id="catear" class="catear"/>
    ...
    <div class="categoryWithCarousel">
        <div class="headbar show1">
            ...
            <div class="rightMenu">
                <span><a href="#nowhere"><img src="${APP_PATH}/img/site/chaoshi.png"/></a></span>
                <span><a href="#nowhere"><img src="${APP_PATH}/img/site/guoji.png"/></a></span>

                <span>
				    <a href="${APP_PATH}/forecategory?cid=83">平板电视</a>
                </span>
                <span>
                    <a href="${APP_PATH}/forecategory?cid=82">马桶</a>
                </span>
                <span>
				    <a href="${APP_PATH}/forecategory?cid=81">沙发</a>
                </span>
                <span>
                    <a href="${APP_PATH}/forecategory?cid=80">电热水器</a>
                </span>
        ...
        <div style="position: relative">
            <script>
                function loadCategory() {
                    $.ajax({
                        type: "POST",
                        url: "${APP_PATH}/getParentCategoryList",
                        async: false,
                        success: function (result) {
                            var catContent = "";
                            var asideContent = "";
                            var parentCats = result.data;
                            //把变量放入到全局变量中，供其他函数调用
                            window.parentCats = parentCats;
                            $.each(parentCats, function (i, parentCat) {

                                catContent += '<div cid="' + parentCat.id + '" class="eachCategory">';
                                catContent += '	<span class="glyphicon glyphicon-link"></span>';
                                catContent += '	<a href="${APP_PATH}/forecategory?cid=' + parentCat.id + '">' + parentCat.name + '</a>';
                                catContent += '</div>';

                                asideContent += '<div cid="' + parentCat.id + '" class="productsAsideCategorys" style="left: 419.5px">';
                                $.each(parentCat.products, function (j, product) {
                                    asideContent += '	<div class="row show1">';
                                    $.each(product.subTitles, function (l, subTitle) {
                                        asideContent += '		<a href="${APP_PATH}/foreproduct?pid=' + product.id + '">' + subTitle + '</a>';
                                    });
                                    asideContent += '		<div class="seperator"></div>';
                                    asideContent += '	</div>';
                                });
                                asideContent += '</div>';
                            });
                            $("div.categoryMenu").html(catContent);
                            $("#asideCat").html(asideContent);
        ...
        <div id="asideCat" style="position: relative;left: 0;top: 0;">
        </div>
        ...
        <div id="carousel-of-product" class="carousel-of-product carousel slide1" data-ride="carousel">
            ...
            <div class="carousel-inner" role="listbox">
                <div class="item active">
                    <img class="carousel carouselImage" src="${APP_PATH}/img/lunbo/1.jpg">
                </div>
                <div class="item">
                    <img class="carouselImage" src="${APP_PATH}/img/lunbo/2.jpg">
                </div>
                <div class="item">
                    <img class="carouselImage" src="${APP_PATH}/img/lunbo/3.jpg">
                </div>

                <div class="item">
                    <img class="carouselImage" src="${APP_PATH}/img/lunbo/4.jpg">
                </div>
    ...
    <div class="homepageCategoryProducts">
        ...
        <script>
            function loadCategoryProducts() {
                var html = "";
                $.each(parentCats, function (i, parentCat) {
                    html += '<div class="eachHomepageCategoryProducts">';
                    html += '	<div class="left-mark"></div>';
                    html += '	<span class="categoryTitle">' + parentCat.name + '</span>';
                    html += '	<div style="clear:both"></div>';
                    $.each(parentCat.products, function (j, product) {
                        html += '	<div class="productItem">';
                        html += '   <a href="${APP_PATH}/foreproduct?pid=' + product.id + '"><img width="100px" src="${APP_PATH}/img/productSingle_middle/' + product.firstProductImage.id + '.jpg"></a>';
                        html += '		<a class="productItemDescLink" href="#nowhere">';
                        html += '			<span class="productItemDesc">[热销]' + product.subTitle + '</span>';
                        html += '		</a>';
                        html += '		<span class="productPrice">￥'+product.promotePrice+'</span>';
                        html += '	</div>';
                    });
                    html += '	<div style="clear:both"></div>';
                    html += '</div>';
                });
                $(".homepageCategoryProducts").html(html);
            }
        </script>

        <img id="endpng" class="endpng" src="${APP_PATH}/img/site/end.png">
        ...
        <div class="modal-content">
            <div class="loginDivInProductPage">
                ...
                <div>
                    ...
                    <a href="regist.jsp" class="pull-right">免费注册</a>
                </div>
