...
<script>
    ...
    function syncSelect() {
        ...
        if (selectAll)
            $("img.selectAllItem").attr("src", "${APP_PATH}/img/site/cartSelected.png");
        else
            $("img.selectAllItem").attr("src", "${APP_PATH}/img/site/cartNotSelected.png");
        ...
        <table class="cartProductTable">
            ...
            <tr>
                <th class="selectAndImage">
                    <img selectit="false" class="selectAllItem" src="${APP_PATH}/img/site/cartNotSelected.png">
                    全选
...
<script>
    ...
    function syncAjax() {
        $.ajax({
            type: "GET",
            url: "/car/findAllUserShopCar/${u.id}",
            success: function (result) {
                console.log(result);
                var orderItem = result.extend.car;
                var content = '';
                window.content = content;
                $(".cartProductTable > tbody").empty();
                $.each(orderItem, function (i, item) {
                    content += '<tr oiid="977" class="cartProductItemTR">'
                    content += '<td>'
                    content += '<img selectit="false" id="'+ item.product.id +'" class="cartProductItemIfSelected" onclick="selection('+item.product.id+','+ item.number +','+ item.product.promotePrice +')"'
                    content += 'src="${APP_PATH}/img/site/cartNotSelected.png">'
                    content += '<a style="display:none" href="#nowhere"><img src="${APP_PATH}/img/site/cartSelected.png"></a>'
                    content += '<img class="cartProductImg" src="${APP_PATH}/img/productSingle_middle/3182.jpg">'
                    content += '</td>'
                    content += '<td>'
                    content += '<div class="cartProductLinkOutDiv">'
                    content += '<a href="/foreproduct?pid='+item.product.id+'" class="cartProductLink">' + item.product.name + '</a>'
                    content += '<div class="cartProductLinkInnerDiv">'
                    ...
                    content += '</td>'
                    content += '<td>'
                    content += '<span class="cartProductItemOringalPrice">￥' + item.product.orignalPrice + '</span>'
                    content += '<span class="cartProductItemPromotionPrice">￥' + item.product.promotePrice + '</span>'
                    content += '</td>'
                    content += '<td>'
                    content += '<div class="cartProductChangeNumberDiv">'
                    content += '<span class="hidden orderItemStock " pid="321">64</span>'
                    content += '<span class="hidden orderItemPromotePrice " pid="321">755.65</span>'
                    content += '<a pid="321" class="numberMinus" href="#nowhere" onclick="numberReduce(' + item.number + ', ' + item.product.id + ', ${u.id})">-</a>'
                    content += '<input pid="321" oiid="977" class="orderItemNumberSetting" autocomplete="off" value="' + item.number + '">'
                    content += '<a stock="64" pid="321" class="numberPlus" href="#nowhere" onclick="numberAdd(' + item.number + ', ' + item.product.id + ', ${u.id})">+</a>'
                    content += '</div>'
                    content += '</td>'
                    content += '<td>'
                    content += '<span class="cartProductItemSmallSumPrice" oiid="977" pid="321">'
                    content += '￥' + (item.number * item.product.promotePrice).toFixed(2) + ''
                    content += '</span>'
                    content += '</td>'
                    content += '<td>'
                    content += '<a class="deleteOrderItem" oiid="977" href="#nowhere" onclick="deleteUserShopCar(' + item.product.id + ', ${u.id})">删除</a>'
                    content += '</td>'
                    content += '</tr>'
                });
                $(".cartProductTable > tbody").html(content);
                sendAjax();
                i = 0;
                j = 0;
                pids = new Array();
                $("span.cartSumNumber").text(j);
                $("span.cartSumPrice").text(i.toFixed(2));
            }
        });
    }
     ...
     *
     */
    function numberReduce(number, pid, uid) {
        if (number == 1) return;
        number--;
        $.ajax({
            type: "GET",
            url: "/car/numberOperation/" + number + "/" + pid + "/" + uid,
            success: function (result) {
                syncAjax();
            }
        });
     ...
     *
     */
    function numberAdd(number, pid, uid) {
        number++;
        $.ajax({
            type: "GET",
            url: "/car/numberOperation/" + number + "/" + pid + "/" + uid,
     ...
     * @param uid
     */
    function deleteUserShopCar(pid, uid) {
        $.ajax({
            type: "GET",
            url: "/car/deleteUserShopCar/" + uid + "/" + pid,
            success: function (result) {
                syncAjax();
            }
        });
    ...
    $("img.selectAllItem").click(function () {
        ...
        if ("selectit" == selectit) {
            ...
            $(".cartProductItemIfSelected").each(function () {
                $(this).attr("src", "${APP_PATH}/img/site/cartNotSelected.png");
            ...
            $(".cartProductItemIfSelected").each(function () {
                $(this).attr("src", "${APP_PATH}/img/site/cartSelected.png");
   ...
   function selection(pid, number, price) {
       var selectit = $("#" + pid);
       var s = selectit.attr("selectit");
       var flag;
       if ("selectit" == s) {
           flag = true;
           totalPrice(pid, number, price, flag);
           selectit.attr("src", "${APP_PATH}/img/site/cartNotSelected.png");
           selectit.attr("selectit", "false");
           selectit.parents("tr.cartProductItemTR").css("background-color", "#fff");
       }
       else {
           flag = false;
           totalPrice(pid, number, price, flag);
           selectit.attr("src", "${APP_PATH}/img/site/cartSelected.png");
           selectit.attr("selectit", "selectit");
           selectit.parents("tr.cartProductItemTR").css("background-color", "#FFF8E1");
       }
   }
    var pids = new Array();
    var i = 0;
    var j = 0;
   function totalPrice(pid, number, price, flag) {
       if (flag) {
           pids.remove(pid);
           i -= (price * number);
           j -= number;
           $("span.cartSumNumber").text(j);
           $("span.cartSumPrice").text(i.toFixed(2));
           $("span.cartTitlePrice").text(i.toFixed(2));
           if (j == 0) {
               $("button.createOrderButton").css("background-color","#AAAAAA");
               $("button.createOrderButton").attr("disabled","disabled");
           }
    ...
    $("button.createOrderButton").click(function () {
        // console.log(pids);
        var param = '/car/settlementShopCar?';
