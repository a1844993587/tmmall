...
<div>
    <a href="#nowhere">
        <img id="simpleLogo" class="simpleLogo" src="${APP_PATH}/img/site/simpleLogo.png">
    </a>
...
<script>
    ...
    $(function () {
        $("a[orderStatus]").click(function () {
            ...
            findCurrentOrderItem(orderStatus);
        });
    ...
    function deleteOrder(id, status) {
        if (status == 1 || status == 4) {
            if (confirm("确定要删除这个订单吗?删除后无法找回哦~")) {
                var orderStatus = $(".selectedOrderType > a").attr("orderStatus");
                /**
                 * 发送ajax请求删除对应订单
                 */
                $.ajax({
     ...
     * @param orderStatus
     */
    function findCurrentOrderItem(orderStatus) {
        /**
         * ajax 发送请求查询
         */
        $.ajax({
            type: "GET",
            url: "/order/findCurrentOrderItem/" + orderStatus + "",
            success: function (result) {
                // console.log(result);
                // 清空内容
                $("#orderListItem").empty();
                var orderItem = result.extend.orderItem;
                var content = "";
                // 判断是否成功
                if (result.code = 100) {
                    // 循环遍历 item
                    $.each(orderItem, function (i, item) {
                        content += '<table class="orderListItemTable" orderStatus="waitReview" oid="965">';
                        content += '<tr class="orderListItemFirstTR">';
                        content += '<td colspan="2">';
                        content += '<b>' + getMyDate(item.createDate) + ' </b>';
                        content += '<span>订单号: ' + item.orderCode + '';
                        content += '</span>';
                        content += '</td>';
                        content += '<td  colspan="2"><img width="13px" src="${APP_PATH}/img/site/orderItemTmall.png">天猫商场</td>';
                        content += '<td colspan="1">';
                        content += '<a class="wangwanglink" href="#nowhere">';
                        content += '<div class="orderItemWangWangGif"></div>';
                        content += '</a>';
                        content += '</td>';
                        content += '<td class="orderItemDeleteTD">';
                        content += '<a class="deleteOrderLink" href="#nowhere" onclick="deleteOrder(' + item.id + ', ' + item.status + ')">';
                        content += '<span  class="orderListItemDelete glyphicon glyphicon-trash"></span>';
                        content += '</a>';
                        content += '</td>';
                        content += '</tr>';
                        var itemsArr = item.orderItems;
                        if (itemsArr.length > 1) {
                            $.each(itemsArr, function (j, items) {
                                if (j > 0) {
                                    content += '</td>';
                                    content += '</tr>';
                                    content += '<tr class="orderItemProductInfoPartTR" >';
                                    content += '<td class="orderItemProductInfoPartTD"><img width="80" height="80" src="${APP_PATH}/img/productSingle_middle/1917.jpg"></td>';
                                    content += '<td class="orderItemProductInfoPartTD">';
                                    content += '<div class="orderListItemProductLinkOutDiv">';
                                    content += '<a href="/foreproduct?pid='+items.product.id+'">' + items.product.name + '</a>';
                                    content += '<div class="orderListItemProductLinkInnerDiv">';
                                    content += '<img src="${APP_PATH}/img/site/creditcard.png" title="支持信用卡支付"> ';
                                    content += '<img src="${APP_PATH}/img/site/7day.png" title="消费者保障服务,承诺7天退货"> ';
                                    content += '<img src=" ${APP_PATH}/img/site/promise.png" title="消费者保障服务,承诺如实描述"> ';
                                    content += '</div>';
                                    content += '</div>';
                                    content += '</td>';
                                    content += '<td  class="orderItemProductInfoPartTD" width="100px">';
                                    content += '<div class="orderListItemProductOriginalPrice">￥' + items.product.orignalPrice + '</div>';
                                    content += '<div class="orderListItemProductPrice">￥' + items.product.promotePrice + '</div>';
                                    content += '</td>';
                                    return true;
                                }
                                content += '<tr class="orderItemProductInfoPartTR" >';
                                ...
                                content += '<div class="orderListItemProductPrice">￥' + items.product.promotePrice + '</div>';
                                content += '</td>';
                                content += '<td valign="top" rowspan="' + item.orderItems.length + '" class="orderListItemNumberTD orderItemOrderInfoPartTD" width="100px">';
                                content += '<span class="orderListItemNumber">' + item.countNumber + '</span>';
                                content += '</td>';
                                content += '<td valign="top" rowspan="' + item.orderItems.length + '" width="120px" class="orderListItemProductRealPriceTD orderItemOrderInfoPartTD">';
                                content += '<div class="orderListItemProductRealPrice">￥' + item.totalPrice + '</div>';
                                content += '<div class="orderListItemPriceWithTransport">(含运费：￥0.00)</div>';
                                content += '</td>';
                                content += '<td valign="top" rowspan="' + item.orderItems.length + '" class="orderListItemButtonTD orderItemOrderInfoPartTD" width="100px">';
                                switch (item.status) {
                                    case "1":
                                        content += '<a href="${APP_PATH}/goAlipay/' + item.id + '">';
                                        content += '<button  class="orderListItemConfirm">付款</button>';
                                        content += '</a>';
                                        break;
                                    case "2":
                                        content += '<span>待发货</span>';
                                        content += '<a href="#nowhere" onclick="eliverGoods(' + item.id + ')">';
                                        content += '<button  class="btn btn-info btn-sm ask2delivery">崔卖家发货</button>';
                                        content += '</a>';
                                        break;
                                    case "3":
                                        content += '<a href="/order/confirmDeal/' + item.id + '">';
                                        content += '<button  class="orderListItemConfirm">确认收货</button>';
                                        content += '</a>';
                                        break;
                                    case "4":
                                        content += '<a href="/order/reviewOrder/' + item.id + '"> ';
                                        content += '<span>待评价</span>';
                                        content += '<button class="orderListItemReview">评价</button>';
                                        content += '</a>';
                                        break;
                                    default:
                                        content += '<a href="#nowhere">';
                                        content += '交易完成';
                                        content += '</a>';
                                }
                                if (j == itemsArr) {
     ...
     * 发货
     */
    function eliverGoods(id) {
        var orderStatus = $(".selectedOrderType > a").attr("orderStatus");
        $.ajax({
            type: "GET",
            url: "/order/eliverGoods/" + id,
            success: function (result) {
                if (result.code == 100) {
                    alert("你慌锤子 已经发货了");
                    findCurrentOrderItem(orderStatus);
...
<div class="boughtDiv">
    ...
    <div class="orderListItem" id="orderListItem">
